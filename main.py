...
def start(update, context):
    ...
    context.bot.send_message(
        ...
        "Этот мир погряз в боли и отчаянии и только "
            ...
            while num <= 1:
                if num == 0:
                    battle = [
                        [
                            InlineKeyboardButton(
                                "Защищаться", callback_data="defense"
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                "Атаковать", callback_data="attack"
                            )
                        ],
                    ]
                    context.bot.send_message(
                        update.effective_chat.id,
                        "Сделай свой выбор",
                        reply_markup=InlineKeyboardMarkup(battle),
                    )
                    if query.data == "attack":
                        context.user_data["enemy"].get_attack(
                            context.user_data["hero"].damage
                        )
                        if context.user_data["enemy"].hp <= 0:
                            if "мешок монет" == new_equip_f_enemy:
                                context.send_message(
                                    f"Я получил {rand_money} монет."
                                )
                            else:
                                context.send_message(
                                    f"Я получил {new_equip_f_enemy}."
                                )
                    if query.data == "defense":
                        context.user_data["hero"].hp += (
                            context.user_data["enemy"].damage + 10
                        )
                        if context.user_data["hero"].hp > 100:
                            context.user_data["hero"].brony += (
                                context.user_data["hero"].hp % 100
                            )
                            context.user_data["hero"].hp = 100
                else:
                    context.user_data["hero"].get_attack(
                        context.user_data["enemy"].damage
                    )
                num = (num + 1) % 2
