...
def start(update, context):
    ...
    context.bot.send_message(update.effective_chat.id,
                             f"Привет, о славный {username}")
    context.bot.send_message(
        update.effective_chat.id,
        f"Этот мир погряз в боли и отчаянии и только ты можешь с этим справиться!",
    )
    start_button = [[
        InlineKeyboardButton("Начать приключение!", callback_data="start")
    ]]
    update.message.reply_text("Готов ли ты дать им отпор",
...
def hello(update, context):
    ...
    if query.data == "start":
        adventure = [
            ...
            [
                InlineKeyboardButton("В поисках приключений!",
                                     callback_data="enemy")
            ],
        ]
        context.bot.send_message(
        ...
        variety = [
            [
                InlineKeyboardButton("Подкрасться со спины",
                                     callback_data="sneak up from behind")
            ],
            [
                InlineKeyboardButton("Подготовить засаду",
                                     callback_data="ambush")
            ],
...
button_check_handler = CallbackQueryHandler(hello)
conversation_handler = ConversationHandler(
    entry_points=[start_handler],
    states={
        HELLO: [start_handler],
        BUTTON_CHECK: [button_check_handler]
    },
    fallbacks=[],
)
